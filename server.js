...
/* If we aren't on Heroku, then we need to adust our port and directory */
    ...
    */
    ...
    socket.on('send_chat_message', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'client did not send a valid room to messge';
            socket.emit('send_chat_message_response', response);
            serverLog('send_chat_message command failed', JSON.stringify(response));
            return;
        }
...
function adjacent_support(who,dr,dc,r,c,board){
    ...
    if (who === 'b'){
        ...
        log("Housten we have a problem:"+who);
        return false;
    }
        ...
        if (sockets.size >= 1) {
            ...
            if((games[game_id].player_white.socket != first) &&
                ...
                if (games[game_id].player_white.socket === ""){
                    ...
                    /* This player should be kicked out - too many players in room */
